...
# Exact 12-word vocabulary specified in assessment
VOCABULARY = [
    "sort",
    "search",
    "binary",
...
# Exact 5 notes specified in assessment
NOTES_DATASET = [
    {
        "id": 1,
...
def mock_embed(text: str) -> List[float]:
    """
    Generates a fixed 12-element numeric vector based on the exact 12-word vocabulary.
    Does NOT use TF-IDF, sklearn, or external embedding libraries.
    """
    if not text:
        return [0.0] * 12
...
def summarize_note(text: Optional[str]) -> Dict[str, Any]:
    """
    Summarizes note text into required JSON format:
    {
      "topic": "...",
      "key_points": [],
      "difficulty": "easy"
    ...
    Difficulty uses lowercase values ("easy", "medium", "hard").
    """
    ...
    # Stop words filter for topic keyword frequency
    stop_words = {
...
def search_notes(query: str) -> List[Dict[str, Any]]:
    """
    Ranks the 5 notes using Cosine Similarity over 12-element vectors generated by mock_embed.
    For empty or OOV queries (resulting in zero vector), returns all 5 notes with score: 0.0.
    """
    ...
    # Sort results descending by score using custom Insertion Sort
    n = len(results)
    for i in range(1, n):
        key_item = results[i]
        j = i - 1
